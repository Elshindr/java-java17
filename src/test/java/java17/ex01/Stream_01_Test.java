...
public class Stream_01_Test {
	...
	public void test_stream_findFirst() throws Exception {
		...
		Optional<Order> result = orders.stream().filter(o->o.getCustomer().getFirstname() == "Sophie").findFirst();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		// pizza Pépéroni
		List<Order> result = orders.stream().filter(
						o -> o.getPizzas().stream().filter(
							p -> p.getName() == "Pépéroni"
						).count() >= 1
		).toList();
